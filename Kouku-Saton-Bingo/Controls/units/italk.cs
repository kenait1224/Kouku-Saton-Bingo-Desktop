...
public class iTalk_NumericUpDown : Control
{
    ...
    protected override void OnPaint(System.Windows.Forms.PaintEventArgs e)
    {
        ...
        int location_x = Width - (int)(Width * 0.32);
        int Button_Width = (int)(Width * 0.32 * 0.8);
        int Button_Top_Padding = (int)(Height * 0.2);
        int Button_Height = Height - Button_Top_Padding;

        G.Clear(Color.Transparent); // Set control background color
        G.FillPath(B1, Shape); // Draw background
        G.DrawPath(P1, Shape); // Draw border

        LinearGradientBrush ColorGradient = new LinearGradientBrush(new Rectangle(location_x, Button_Top_Padding, Button_Width, Button_Height - Button_Top_Padding + 1), Color.FromArgb(241, 241, 241), Color.FromArgb(241, 241, 241), 90.0F);
        G.FillRectangle(ColorGradient, ColorGradient.Rectangle); // Fills the body of the rectangle

        G.DrawRectangle(new Pen(Color.FromArgb(252, 252, 252)), new Rectangle(location_x , Button_Top_Padding -1, Button_Width, Button_Height-Button_Top_Padding + 1));
        G.DrawRectangle(new Pen(Color.FromArgb(180, 180, 180)), new Rectangle(location_x, Button_Top_Padding-1, Button_Width, Button_Height - Button_Top_Padding + 1));

        G.DrawLine(new Pen(Color.FromArgb(250, 252, 250)), new Point(location_x + 1, Height / 2), new Point(Width - 5, Height / 2));
        G.DrawLine(new Pen(Color.FromArgb(180, 180, 180)), new Point(location_x + 1, Height / 2 -1), new Point(Width - 5, Height / 2 - 1));
        G.DrawLine(new Pen(Color.FromArgb(250, 250, 250)), new Point(location_x + 1, Height / 2 -2), new Point(Width - 5, Height / 2 - 2));

        G.DrawString("+", new Font("Tahoma", 8), Brushes.Gray, location_x + (int)(Button_Width / 4) - 1, 1);
        G.DrawString("-", new Font("Tahoma", 12), Brushes.Gray, location_x + (int)(Button_Width / 4) - 1, Height/3 - 1);
...
public class iTalk_ProgressBar : Control
{
    ...
    private Color _ProgressColor3 = Color.FromArgb(52, 52, 52);
            ...
            using (Graphics graphics = Graphics.FromImage(bitmap))
            {
                ...
                using (LinearGradientBrush brush = new LinearGradientBrush(this.ClientRectangle, this._ProgressColor1, this._ProgressColor2, LinearGradientMode.ForwardDiagonal))
                {
                    using (Pen pen = new Pen(brush, 14f))
                    {
                        ...
                        graphics.DrawArc(pen, 0x12, 0x12, (this.Width - 0x23) - 2, (this.Height - 0x23) - 2, -90, (int)Math.Round((double)((360.0 / ((double)this._Maximum)) * (this._Maximum -this._Value))));
                    }
                }
                using (LinearGradientBrush brush2 = new LinearGradientBrush(this.ClientRectangle, this._ProgressColor3, this._ProgressColor3, LinearGradientMode.Vertical))
                {
                    graphics.FillEllipse(brush2, 0x18, 0x18, (this.Width - 0x30) - 1, (this.Height - 0x30) - 1);
                }
                SizeF MS = graphics.MeasureString(Convert.ToString(Convert.ToInt32(_Value)), Font);
                graphics.DrawString(Convert.ToString(Convert.ToInt32(_Value)), Font, Brushes.White, Convert.ToInt32(Width / 2 - MS.Width / 2), Convert.ToInt32(Height / 2 - MS.Height / 2));
...
class iTalk_TabControl : TabControl
{
    ...
    int TabString_LocationY_offset;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        _Graphics.FillRectangle(new SolidBrush(Color.White), new Rectangle(-5, 0, ItemSize.Height + 4, Height));
        // Draw vertical line at the end of the tab selector rectangle
        _Graphics.DrawLine(new Pen(Color.FromArgb(202, 207, 215)), ItemSize.Height - 1, 0, ItemSize.Height - 1, Height);
        _Graphics.DrawLine(new Pen(Color.FromArgb(202, 207, 215)), 0, 0, Width, 0);
        ...
        for (int TabIndex = 0; TabIndex <= TabCount - 1; TabIndex++)
        {
            if (TabIndex == SelectedIndex)
            {
                ...
                _Graphics.FillRectangle(new SolidBrush(Color.FromArgb(245, 245, 245)), TabRect.X, TabRect.Y + 1, TabRect.Width - 4, TabRect.Height + 3);
                ...
                _Graphics.FillRectangle(new SolidBrush(Color.FromArgb(177, 182, 180)), TabHighlighter);
                // Draw tab text
                _Graphics.DrawString(TabPages[TabIndex].Text, new Font(Font.FontFamily, Font.Size, FontStyle.Regular), new SolidBrush(Color.FromArgb(47, 47, 47)), new Rectangle(TabRect.Left, TabRect.Top + ItemSize.Width / 2 - (int)Font.Height / 2 - 3, TabRect.Width , TabRect.Height), new StringFormat { Alignment = StringAlignment.Center });
            ...
            {
                Rectangle TabRect = new Rectangle(new Point(GetTabRect(TabIndex).Location.X - 2, GetTabRect(TabIndex).Location.Y - 2), new Size(GetTabRect(TabIndex).Width + 3, GetTabRect(TabIndex).Height - 8));
                _Graphics.DrawString(TabPages[TabIndex].Text, new Font(Font.FontFamily, Font.Size, FontStyle.Regular), new SolidBrush(Color.FromArgb(100, 100, 100)), new Rectangle(TabRect.Left, TabRect.Top + ItemSize.Width/ 2 - (int)Font.Height / 2 -3, TabRect.Width , TabRect.Height), new StringFormat { Alignment = StringAlignment.Center });
